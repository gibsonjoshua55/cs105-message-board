...
app.post('/post-message', function(req, res){
	...
	var passwordError = !(req.body.password === process.env.SECRET_PASSWORD);
	...
	else
	{
		...
		VALUES (?,?,?,?,?,?,?)`, [message.salutation, message.posted_by, message.topic, message.message, message.color, message.shout, new Date()],
